...
/*   Updated: 2024/01/05 01:56:59 by suminkwon        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char *find_newline(char *previous_read, int fd)
{
	...
	while (readsize && !ft_strchr(previous_read, '\n'))
	{
		...
		if (!buffer)
		{
			// printf("buffer malloc error in the findnewline\n");
			return (free_one(&previous_read));
		}
		readsize = read(fd, buffer, BUFFER_SIZE);
		if (readsize == -1)
		{
			// printf("reading error in the findnewline\n");
...
static int store_line(char *previous_read, char **line)
{
	...
	if (!previous_read[i])
	{
		// printf("!previous_read[i] == '\\0' in leftover func\n");
		return (-1);
	}
	...
	*line = (char *)malloc(sizeof(char) * (i + 1));
	if (!*line)
	{
		// printf("line unmalloc in leftover func\n");
		return (-1);
	}
	i = 0;
	while (previous_read[i] && previous_read[i] != '\n')
...
{
	static char *previous_read[OPEN_MAX]; // 문자열을 담을수 있는 previous가 fd열린만큼의 갯수 만큼 있어야지
	char *line;
	...
	// printf("First : previous_read[%d] = %s\n", fd, previous_read[fd]);
	previous_read[fd] = find_newline(previous_read[fd], fd);
	// printf("Second : previous_read[%d] = %s\n", fd, previous_read[fd]);
	if (!previous_read[fd])
		return (NULL);
